    ...
    <div className="h-full overflow-y-auto p-6">
      <div className="mx-auto grid max-w-7xl gap-6 lg:grid-cols-[minmax(0,1fr)_420px]">
        <section className="space-y-5 border border-border bg-card/60 p-5">
          <div className="flex items-center gap-3"><FileEdit className="h-5 w-5 text-primary" /><div><h2 className="font-semibold tracking-wider">DETERMINISTIC RENAME</h2><p className="text-xs text-muted-foreground">Natural ordering, collision preflight, atomic copies, and resumable manifests</p></div></div>
          <div className="grid gap-4 md:grid-cols-2"><Input label="Input images" value={loadPath} onChange={setLoadPath} type="path" onBrowse={browseLoad} disabled={isActive} /><Input label="Output directory" value={savePath} onChange={setSavePath} type="path" onBrowse={browseSave} disabled={isActive} /></div>
