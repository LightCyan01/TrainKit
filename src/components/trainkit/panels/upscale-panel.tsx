        ...
        <section className="space-y-5 border border-border bg-card/60 p-5">
          ...
          {backend === "ncnn" && (
            <div className="space-y-4 border border-accent/20 bg-accent/5 p-4">
              <Input label="NCNN .bin weights" value={modelBinPath} onChange={setModelBinPath} type="path" onBrowse={browseBin} disabled={isActive} />
              <div className="grid gap-3 sm:grid-cols-3">
                <label className="space-y-2"><span className="text-[10px] uppercase text-muted-foreground">Input blob</span><input value={inputBlob} onChange={(event) => setInputBlob(event.target.value)} className="w-full border border-border bg-input px-3 py-2 text-sm" /></label>
                <label className="space-y-2"><span className="text-[10px] uppercase text-muted-foreground">Output blob</span><input value={outputBlob} onChange={(event) => setOutputBlob(event.target.value)} className="w-full border border-border bg-input px-3 py-2 text-sm" /></label>
          ...
          <ModelStatus modelName={modelInfo?.name ?? modelPath.split(/[\\/]/).pop()} isLoaded={Boolean(modelInfo)} loadState={inspecting ? "loading" : modelInfo ? "loaded" : "idle"} details={modelInfo ? `${modelInfo.architecture} | ${modelInfo.scale}x${backend === "ncnn" && modelInfo.vulkan ? " | Vulkan" : ""}` : "Inspection is explicit; selecting a file never deserializes it"} />
          <div className="grid gap-4 md:grid-cols-2"><Input label="Input images" value={loadPath} onChange={setLoadPath} type="path" onBrowse={browseLoad} disabled={isActive} /><Input label="Output images" value={savePath} onChange={setSavePath} type="path" onBrowse={browseSave} disabled={isActive} /></div>
